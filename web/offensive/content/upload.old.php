...
#	$filePath=dirname(realpath($_SERVER['SCRIPT_FILENAME']));
	...
	if( $_FILES['image']['name'] != "" && $_FILES['image']['size'] > 0 && ! exceededUploadLimit() ) {
		...
		if( ! preg_match( "/(jpg|jpeg|gif|png|mp3|ogg)/i", $fileinfo["fileformat"] ) ) {
			...
			echo "Unaccepted file type. Accepted file types are: jpg, gif, png, mp3, ogg. Your actions are being monitored.";
		...
		if( is_array( $matches ) ) {
			...
			if( ! preg_match( "/(jpg|jpeg|gif|png|mp3|ogg)/i", $file_extension ) ) {
				?>Accepted file types: jpg, gif, png, mp3, ogg.<?php
				exit;
			}
		}
...
#		mail( "ray@mysocalled.com", "[" . $_SERVER["REMOTE_ADDR"] . "] - file uploaded. " . $_SESSION['username'], $filename . "\n\nhttp://themaxx.com/offensive/pages/pic.php?id=" . $entry_id, "From: offensive@themaxx.com (this might be offensive)" );
	...
	function filenameFor( $fileinfo, $default ) {
		...
		if( $fileinfo["fileformat"] == "mp3" || $fileinfo["fileformat"] == "ogg" ) {
			if( isset( $fileinfo['comments_html']['artist'] ) && isset( $fileinfo['comments_html']['title'] ) ) {
				return (implode( $fileinfo['comments_html']['artist'], " " ) . " - " . implode( $fileinfo['comments_html']['title'], "" ) ) . "." . $fileinfo["fileformat"];
			}
		}
	...
	function typeFor( $format ) {
		return ( $format == "mp3" || $format == "ogg" ) ? "audio" : "image";
	}

	function pathFor( $format ) {
		global $filePath;
		return $filePath . (($format == "mp3" || $format == "ogg") ?  "/images/audio/" : "/images/picpile/");
	}
	...
	function body() {
		...
		if( isset( $repost_row ) ) {
			...
			$message = "Your upload exceeds the recommended file size for an image of its dimensions. Expected file size: ${expectedFileSize}k. Your file: ${filesize}k. (If you'd like to compress it but don't have the tools, you might try running it through <a target=\"_blank\" href=\"http://lostcause.porkchop.net/~porkchop/downsize/\">porkchop's magic maxx photo prep thing</a>. (thanks porkchop!))<br/>Post it anyway?</br>";
			repostForm( $message, $hash );			
		}
		else if( $width > 1200 || $height > 1200 ) {
			$message = "This image is rather large ($width x $height). (If you'd like to resize it but don't have the tools, you might try running it through <a target=\"_blank\" href=\"http://lostcause.porkchop.net/~porkchop/downsize/\">porkchop's magic maxx photo prep thing</a>. (thanks porkchop!))<br/>Post it anyway?";
			repostForm( $message, $hash );			
		}
		else {
