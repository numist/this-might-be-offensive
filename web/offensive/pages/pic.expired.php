<?php session_start();
	...
	function writeNav( $id ) {
		...
		global $filename, $nsfw, $tmbo, $uploader, $uploaderid, $timestamp;
		...
		if( ! is_numeric( $nextid ) ) {
			...
			if( is_numeric( $nextid ) ) {
				writeNavFile( "nav/$id.php", $filename, $nextid, $previd, $nsfw, $tmbo, $uploader, $uploaderid, $timestamp );
			}
		}
	...
	function writeNavFile( $navfile, $filename, $nextid, $previd, $nsfw, $tmbo, $uploader, $uploaderid, $timestamp ) {
		$escaped_fname = str_replace( "\"", "\\\"", $filename );
			...
			<?
				if( ! file_exists( "../images/picpile/$filename" ) ) {
					...
					if( $_SESSION['prefs']['hide nsfw'] == 1 && $nsfw == 1
						...
						?>
						<div class="<?php echo $nsfw == 1 ? 'nsfw' : 'image' ?> u<?= $uploaderid ?>"><a href="../images/picpile/<? echo rawurlencode( $filename )?>" target="_blank"><img src="http://images.tmbo.org/mirror.php/offensive/images/picpile/<? echo rawurlencode( $filename )?>" style="border:none"/></a></div>
