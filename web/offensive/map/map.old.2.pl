...
sub makeUserMap {
	...
	my $crop_x = $x - ($cropped_size/2);
	my $crop_y = $y - ($cropped_size/2);

	$minimap->Crop( geometry=>'250x250', width=>$cropped_size, height=>$cropped_size, x=>$crop_x, y=>$crop_y );
